...
class SimpleCNN(nn.Module):
    ...
    def __init__(self, num_classes = NUM_OF_CLASSES):
        ...
        self.features = nn.Sequential(
            ...
            # ----------- conv-2 -----------
            # Change from AlexNet Conv2d(96, 256, 5, padding=2) -> Conv2d(64, 192, 5, padding=2)
            ...
            # ----------- conv-3 -----------
            # Change from AlexNet Conv2d(256, 384, 3, padding=1) -> Conv2d(192, 256, 3, padding=1)
            nn.Conv2d(192, 256, 3, padding=1),
            # shape = (B, 256, 12, 12)
            nn.ReLU(),

            # ----------- conv-4 -----------
            # Change from AlexNet Conv2d(384, 384, 3, padding=1) ->Conv2d(256, 256, 3, padding=1)
            nn.Conv2d(256, 256, 3, padding=1),
            # shape = (B, 256, 12, 12)
            nn.ReLU(),

            # ----------- conv-5 -----------
            # Change from AlexNet Conv2d(384, 256, 3, padding=1) ->Conv2d(256, 256, 3, padding=1)
            nn.Conv2d(256, 256, 3, padding=1),
            # shape = (B, 256, 12, 12)
            nn.ReLU(),
            nn.MaxPool2d(3, 2),
        ...
        self.classifier = nn.Sequential(
            ...
            # Change from AlexNet (4096, 4096) -> (1024, 1024)
            nn.Linear(1024, 1024, dtype=DTYPE),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.5),

            # Change from AlexNet (1024, 1000) -> (1024, num_classes)
            nn.Linear(1024, num_classes, dtype=DTYPE)
        )
