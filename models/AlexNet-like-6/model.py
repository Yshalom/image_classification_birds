...
class AlexNetLike(nn.Module):
    ...
    def __init__(self, num_classes = NUM_OF_CLASSES):
        ...
        self.classifier = nn.Sequential(
            # Change from AlexNet (6400, 4096) -> (6400, 2048)
            nn.Linear(6400, 2048, dtype=DTYPE),
            nn.ReLU(),
            nn.Dropout(p=0.5),

            # Change from AlexNet (4096, 4096) -> (2048, 2048)
            nn.Linear(2048, 2048, dtype=DTYPE),
            nn.ReLU(),
            nn.Dropout(p=0.5),

            # Change from AlexNet (2048, 1000) -> (2048, num_classes)
            nn.Linear(2048, num_classes, dtype=DTYPE)
        )
